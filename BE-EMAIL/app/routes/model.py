...
async def preview_dataset(
    file: UploadFile = File(..., description="CSV/Excel file untuk dipreview"),
):
    """Membaca file untuk mendapatkan jumlah spam dan ham tanpa menyimpannya ke database."""
    ...
    try:
        ...
        if "label" not in df.columns:
            raise HTTPException(
                status_code=400, 
                detail="Dataset harus memiliki kolom 'label'."
            )
        ...
        raise HTTPException(status_code=500, detail=f"Gagal preview dataset: {str(e)}")
...
async def upload_dataset(
    ...
    dataset_name: Optional[str] = Form(None, description="Nama custom untuk dataset"),
    db: Session = Depends(get_db),
):
    """
    ...
    try:
        ...
        ham_count = 0
        new_dataset = Dataset(
            name=dataset_name if dataset_name and dataset_name.strip() else file.filename,
